...
struct Args {
    /// Network interface to bind. Defaults to feth0 (standard dev setup).
    #[arg(short, long, default_value = "feth0")]
    iface: String,
    #[arg(long, default_value_t = 4)]
    players: u8,
    /// Path to a Rekordbox USB export root. Optional -- tracks can be left
    /// empty and the UI will show decks with no track loaded.
    #[arg(long)]
...
async fn main() -> anyhow::Result<()> {
    ...
    // CLI arg takes priority; fall back to CDJ_LIBRARY env var for dev convenience.
    let library_root = args.library
        .or_else(|| std::env::var("CDJ_LIBRARY").ok().map(std::path::PathBuf::from));

    let library = if let Some(root) = library_root {
